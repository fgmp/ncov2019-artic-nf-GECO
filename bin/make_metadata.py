...
# Read local_id as pandas dataframe.
locid_dict = pd.read_csv(local_ids, sep=",", header=0, index_col=0).to_dict()
...
## Assign central_id's, gisaid_name's to summary_df based on last used local_id of record on REDCap.
# Fetch local_id and central_id of records from REDCap.
project = redcap.Project(url, token)
fields_of_interest = ['central_id', 'local_id']
central_id_df = project.export_records(fields=fields_of_interest, format='df')
central_id_df["central_id_col"] = central_id_df.index.to_series()
central_id_df = central_id_df[["local_id", "central_id_col"]]

# Merge central_id_df with summary_df on local_id.
summary_df = summary_df.merge(central_id_df, how='left', left_on='local_id', right_on='local_id')
summary_df["central_id"] = summary_df["central_id_col"]
summary_df = summary_df.drop(columns=["central_id_col"])

# Loop summary_df rows.
for index, row in summary_df.iterrows():
    # Set gisaid_name of summary_df record using central_id column value.
    summary_df.loc[index, "gisaid_name"] = "PH"+"-"+dag.upper()+"-"+row["central_id"]
